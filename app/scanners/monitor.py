...
class MrktMonitor:
    ...
    def _calc_floor(gifts: list) -> dict[str, float]:
        """MRKT не отдаёт floor_price — считаем минимум по коллекции из выборки."""
        ...
        for g in gifts:
            ...
            try:
                ...
                continue
            if name not in floors or price < floors[name]:
                floors[name] = price
        return floors
    ...
    async def _fetch_gifts(self, token: str) -> list:
        ...
        inner = {
            ...
            "ordering":        "Price",   # обязательно строка, null не принимается
        ...
        # API требует обёртку {"req": <тело запроса>}
        payload = {"req": inner}
        async with AsyncSession() as s:
            ...
            # ответ может быть {"gifts": [...]} или напрямую списком
        ...
        try:
            ...
            for g in gifts:
                ...
                listing = Listing(
                    ...
                    model_rarity=float(model.get("rarityPercent") or 0),
                    backdrop=backdrop.get("name", ""),
                    backdrop_rarity=float(backdrop.get("rarityPercent") or 0),
                    symbol=symbol.get("name", ""),
                    symbol_rarity=float(symbol.get("rarityPercent") or 0),
                    listed_at=g.get("createdAt", ""),
                    photo_url=g.get("previewUrl", ""),
